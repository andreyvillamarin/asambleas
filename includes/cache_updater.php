...
function update_meeting_cache() {
    ...
    if (!$meeting) {
        // Si no hay reunión activa, el estado es "cerrada".
        $cached_data = ['status' => 'closed'];
    } else {
        // --- INICIO DE CÁLCULOS (se ejecuta solo cuando hay reunión) ---
    ...
    // --- ESCRITURA ATÓMICA ---
    // Escribir los datos en un archivo temporal y luego renombrarlo para garantizar
    // que los procesos que leen la caché nunca encuentren un archivo incompleto.
    $temp_cache_file_path = $cache_file_path . '.tmp';
    
    // Usar JSON_PRETTY_PRINT solo para la estructura de datos compleja de la reunión activa.
    $json_options = ($meeting) ? JSON_PRETTY_PRINT : 0;
    $json_data = json_encode($cached_data, $json_options);

    if ($json_data !== false && file_put_contents($temp_cache_file_path, $json_data) !== false) {
